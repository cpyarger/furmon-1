...
def opstate(input):
	...
	# print state 
	x=0
	statepairs = [ ('IDL','idle') , ('I','idle') , ('IGN','igniting') , ('FEE','feedstart'),('FIR','firing'),('STA','stabilizing'),('S','shuttingdown'),('PUR','purging'),('ALA','alarm') ]
	for s in statepairs:
...
def dofile():
	...
	print '%s\t%s\t%s\t%s\t%s' % (frfile.recno,frfile.hours,frfile.system_temp,frfile.stack_temp,frfile.state)
	#d while ( i < 8 ):
 	while True:
 	...
 	# while rfdl(dline):
		...
		while ( b < len(dataline)):
			if ( len(dataline[b]) < 50 ):  # sets allowable element length
				...
				if (ophours(l) != "" ):		#Check for number of hours in this element
					frfile.hours = ophours(l)
				else:
					frfile.hours = ""
				if (opstate(l) != "" ):		#Check for operational state in this element
