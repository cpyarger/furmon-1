...
def dofile():
	...
	flue="flue"
	i = 0
	print '%s\t\t\t%s' % (burner,flue)
	# print '%i\t%s\t\t\t%s' % (i,burner,flue)
	# while ( i < 8 ):
 	while True:
	 	i += 1
		b = 1
		...
		while ( b < len(dataline)):
			if ( len(dataline[b]) < 10 ):
				...
				if ( str(''.join([c for c in l if l.startswith('B')]))[0:] ):
					...
					burner = re.sub(r'[^\w]', '',(str(''.join([c for c in l if l.startswith('B')]))[1:4]))
				if ( str(''.join([c for c in l if l.startswith('F')]))[0:] ):
					# flue = (str(''.join([c for c in l if l.startswith('F')]))[1:4])
					flue   = re.sub(r'[^\w]', '',(str(''.join([c for c in l if l.startswith('F')]))[1:4]))
				if ( burner != "burner" and flue != "flue"):
					print '%s\t\t\t%s' % (''.join(c for c in burner if c in digits),''.join(c for c in flue if c in digits))
	 		b += 1
#
def cleanv(input,trigger):
	''' cleans up a variable returns a number no matter what'''
	mid = re.sub(r'[^\w]', '',(str(''.join([c for c in l if input.startswith(trigger)]))[1:4]))
